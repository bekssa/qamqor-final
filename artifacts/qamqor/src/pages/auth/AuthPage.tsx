...
export default function AuthPage() {
  ...
  const [login2, setLogin2] = useState({ credential: "", password: "" });
  ...
  const validateLogin = () => {
    const errors: Record<string, string> = {};
    if (!login2.credential.trim()) errors.credential = t("auth.required");
    if (!login2.password) errors.password = t("auth.required");
    else if (login2.password.length < 8) errors.password = t("auth.passwordMin");
    return errors;
  };
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    ...
    if (Object.keys(errors).length > 0) return;
    const ok = login(login2.credential, login2.password);
    if (!ok) {
  ...
  const roleOptions = [
    { value: "elderly", label: t("auth.roleElderly") },
    { value: "volunteer", label: t("auth.roleVolunteer") },
    { value: "relative", label: t("auth.roleRelative") },
  ];
            ...
            <form onSubmit={handleLoginSubmit} className="flex flex-col gap-4" noValidate>
              <div className="flex flex-col gap-1">
                ...
                <input
                  ...
                  value={login2.credential}
                  onChange={(e) => setLogin2((p) => ({ ...p, credential: e.target.value }))}
                  className={`w-full px-4 py-3 rounded-xl border text-sm outline-none transition-all
              ...
              <div className="flex flex-col gap-1">
                ...
                <div className="relative">
                  <input
                    type={showPassword ? "text" : "password"}
                    placeholder={t("auth.passwordPlaceholder")}
                    value={login2.password}
                    onChange={(e) => setLogin2((p) => ({ ...p, password: e.target.value }))}
